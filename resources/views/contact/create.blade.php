...
@section('content')
    ...
    <div class="col-span-12">
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <!-- Form -->
            <form action="{{ route('contact.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                <div class="space-y-6">
                    ...
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                            {{ __('messages.birthday')  }} <span class="text-error-500">*</span>
                        </label>
                            ...
                            <div class="flex-1">
                                ...
                                >
                                    ...
                                    @foreach(\App\Enums\Constants::months() as $value => $monthKey)
                                        <option value="{{ $value }}" {{ old('birthday_month') == $value ? 'selected' : '' }}>
...
@push('scripts')
    <script>
        ...
        window.removeImage = function() {
            ...
            // Only remove if file is selected
            if (imageInput.files.length > 0 || avatarPreview.querySelector('img')) {
                // Clear file input
                imageInput.value = '';

                // Reset avatar to default SVG
                avatarPreview.innerHTML = '<svg class="h-16 w-16 text-gray-400 dark:text-gray-600" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14.2c-2.5 0-4.71-1.28-6-3.22.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08-1.29 1.94-3.5 3.22-6 3.22z"/></svg>';

                // Hide success badge
                successBadge.classList.add('hidden');

                // Reset border to default
                avatarContainer.classList.remove('border-success-500', 'border-4');
                avatarContainer.classList.add('border-gray-200', 'dark:border-gray-700');
            }
        };
    </script>
@endpush